        ...
        while True:
            ...
            if start_window_event == 'Entrar':
                ...
                while True:
                    ...
                    if forgot_password_event == sg.WIN_CLOSED:
                        control = False
                        break

                    elif forgot_password_event == 'Enviar':
                        
                        if check_user(values[0]):
                            send_mail(values[0])
                            forgot_pass_window.close()
                            forgot_password_success_window = forgot_password_success_interface()
                            forgot_password_success_event, values = forgot_password_success_window.read()    

                            if forgot_password_success_event == 'Voltar':
                                forgot_password_success_window.close()
                                break

                            elif forgot_password_success_event == sg.WIN_CLOSED:
                                control = False
                                forgot_password_success_window.close()
                                break
